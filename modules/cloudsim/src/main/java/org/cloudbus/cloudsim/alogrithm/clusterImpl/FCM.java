...
import org.cloudbus.cloudsim.alogrithm.clusterImpl.meta.Fcm;
...
public class FCM<T> {
    ...
    private Double[][] generateCenter() {
        ...
        System.out.println("centers:" + centers[0].length);
        return centers;
    }
    ...
    private void dictCluster() {
        Fcm fcm = new Fcm(k, 200, 2, 1e-6);
        fcm.setCenters(generateCenter());
        fcm.fit(data);
        Double[][] centers = fcm.getCenters();
        List<Domain> result = fcm.getCluster();
