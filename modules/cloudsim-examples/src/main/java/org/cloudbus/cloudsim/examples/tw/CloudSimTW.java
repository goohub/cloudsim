...
import org.cloudbus.cloudsim.alogrithm.clusterImpl.TriCluster;
...
public class CloudSimTW {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            //train by tricluster
            TriCluster<PowerVm> tc = new TriCluster<PowerVm>(4);
            tc.fit(vmlist);
            tc.adapt();
    ...
    private static List<PowerVm> createVms(int brokerId, int vmsNumber) {
        ...
        for (int i = 0; i < ConstantsConf.NUMBER_VMS; i++) {
            int vmType = i / (int) Math.ceil((double) vmsNumber / ConstantsConf.VM_TYPES);
    ...
    private static List<Cloudlet> createCloudlets(int brokerId, int numberCloudlets) {
        ...
        for (int i = 0; i < numberCloudlets; i++) {
            long length = (long) (ConstantsConf.CLOUDLET_LENGTH * random.nextDouble()) + 400L;
            long outputSize = (long) (ConstantsConf.CLOUDLET_OUTPUTSIZE * random.nextDouble()) + 20L;
            Cloudlet cloudlet = new Cloudlet(i, length, ConstantsConf.CLOUDLET_PES, fileSize, outputSize, utilizationModel, utilizationModel, utilizationModel);
